...
*   **灵感来源**: 深入 `analysis_report.json`，寻找最戏剧性的冲突点：
    *   **玩家对决**: 是不是"知名游资"和"量化基金"在掰手腕？
    *   **多空反差**: 是不是"多方胜利"的表象下，主力净买入其实很少？
    *   **行为矛盾**: 是不是K线在"趋势加速"，但某个关键先生却在悄悄卖出？
    *   **情绪与现实**: 是不是市场情绪"亢奋"，但卖盘压力巨大？
...
*   **人设**: 生成一个符合「激进的游资跟风者」的昵称，要生动、接地气，避免AI味。风格兴奋、激进，多用感叹号和短句。
    *   **灵感范例**: "板上饮酒", "核按钮舵手", "龙头信仰者", "一进二专业户"
*   **视角**: 只看多方优势。聚焦JSON中的积极信号。
*   **内容**: 直接喊出看多的理由。是什么让这只票看起来这么诱人？哪个大佬进场了？K线走出什么牛逼形态了？

#### **角色二：空头提醒 (`bear_comment`)**

*   **人设**: 生成一个符合「风控专家」风格的昵称，可以带点调侃或自嘲。风格理性、谨慎，注重逻辑，条理清晰。
    *   **灵感范例**: "K线找茬王", "三根阴线就跑", "价值守望者", "高位测谎仪"
...
*   **人设**:
    *   **提问者**: 生成一个符合「散户新人」风格的昵称，能体现出谦虚、困惑或希望。
        *   **灵感范例**: "股市小萌新", "求回本的韭菜", "今天站岗了吗", "想回本的打工人"
    *   **回答者**: 保持核心"顶级操盘手"人设进行回复，无需昵称。
*   **内容**:
    *   **第一步：新手提问 (`question`)**: 提出一个能串联起多空双方矛盾点的"真问题"。这个问题应该能体现思考，而不是简单地问"该不该买"。
    *   **第二步：操盘手回答 (`answer`)**: 针对这个问题，进行一次"授人以渔"式的点拨。教方法论。重点是提供一个观察框架和思考路径。
...
"""
    ...
    def generate_stage2_content(self, analysis_data, stage1_content):
        ...
        json_schema = """{
    "title": "...",
    "bull_comment": {"nickname": "...", "content": "..."},
    "bear_comment": {"nickname": "...", "content": "..."},
    "QA": {"questioner": {"nickname": "...", "content": "..."}, "answerer": {"content": "..."}}
}"""
    ...
    def format_comments_json_to_markdown(self, json_result):
        ...
        bull_nickname = bull_comment.get("nickname", "涨停敢死队")
        ...
        bear_nickname = bear_comment.get("nickname", "稳健投资者")
        ...
        questioner_nickname = questioner.get("nickname", "小白学股")
...
def test_generator():
    ...
    try:
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        workspace_root = os.path.dirname(current_dir)
        json_file_path = os.path.join(workspace_root, "data", "output", "funding_battle_analysis_report-5.json")
